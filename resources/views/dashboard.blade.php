...
<body class="min-h-screen bg-gray-100">
        ...
        <div class="mx-auto max-w-7xl py-6 px-4 sm:px-6 lg:px-8 flex justify-between items-center">
            ...
            <div class="flex items-center gap-4">
                ...
                <form method="POST" action="{{ route('logout') }}" class="inline">
                    @csrf
                    <button type="submit" class="rounded-md bg-gray-200 px-3 py-1.5 text-sm font-medium text-gray-700 hover:bg-gray-300">
                        {{ __('Log out') }}
                    </button>
                </form>
            </div>
    ...
    <main class="mx-auto max-w-7xl py-6 sm:px-6 lg:px-8">
        <div class="rounded-lg border border-gray-200 bg-white px-6 py-8 shadow-sm">
            <p class="text-lg text-gray-900">{{ __('Dashboard Loaded') }}</p>
            <p class="mt-2 text-sm text-gray-500">{{ __('You are logged in.') }}</p>
        </div>
    </main>
</body>
</html>
